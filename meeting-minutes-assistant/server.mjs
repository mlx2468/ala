...
function normalizeProvider(provider = {}) {
  const normalizedChatEndpoint = normalizeChatEndpoint(provider.chatEndpoint || provider.endpoint || defaultProvider.chatEndpoint);
  const transcribeEndpoint = provider.transcribeEndpoint === "浏览器实时识别，无需填写" ? "" : provider.transcribeEndpoint;
  const normalizedTranscribeEndpoint = normalizeTranscribeEndpoint(transcribeEndpoint);
  return {
    ...defaultProvider,
    chatEndpoint: normalizedChatEndpoint,
    transcribeEndpoint: normalizedTranscribeEndpoint || defaultProvider.transcribeEndpoint,
...
async function handleApi(req, res, pathname) {
  ...
  if (pathname === "/api/settings/provider" && req.method === "PUT") {
    ...
    const normalizedBodyTranscribeEndpoint = normalizeTranscribeEndpoint(body.transcribeEndpoint);
    if (normalizedBodyChatEndpoint && !/^https:\/\//i.test(normalizedBodyChatEndpoint)) {
      return sendJson(res, 400, { error: "模型服务地址必须是 HTTPS 地址。" });
    }
    if (normalizedBodyTranscribeEndpoint && !/^https:\/\//i.test(normalizedBodyTranscribeEndpoint)) {
    ...
    db.provider = normalizeProvider({
      ...
      transcribeEndpoint: body.transcribeEndpoint === undefined ? db.provider.transcribeEndpoint : normalizedBodyTranscribeEndpoint,
