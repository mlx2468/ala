...
const i18n = {
  "zh-CN": {
    ...
    modelEndpoint: "模型服务地址",
    minutesModel: "纪要模型",
    transcribeEndpoint: "转写服务地址",
...
function openModal({ title, description = "", inputValue = "", confirmText = "确定", cancelText = "取消", danger = false, prompt = false }) {
  closeModal(false);
...
async function loadProvider() {
  ...
  $("transcribeEndpointInput").value = provider.transcribeEndpoint || "";
...
async function transcribeAudio(file) {
  if (!file) return;
  if (!state.provider?.demoMode && !state.provider?.transcribeEndpoint) {
    toast("未配置服务端转写接口。实时录音请用“开始录音”；上传音频需先到设置页填写 HTTPS 转写接口。", "error");
    activateView("settings");
    return;
  }
  const formData = new FormData();
  formData.append("audio", file);
  $("recordState").textContent = "正在转写文件";
  qs(".record-card")?.setAttribute("data-active", "true");
  try {
    ...
    $("sourceMode").textContent = result.source === "api" ? "模型转写" : "浏览器/演示转写";
    toast("转写内容已写入左侧文本区");
  } catch (error) {
    toast(error.message, "error");
  } finally {
    $("recordState").textContent = "待录入";
    qs(".record-card")?.removeAttribute("data-active");
  }
}
...
function renderSettingsView() {
    ...
    <form id="workspaceSettingsForm" class="settings-page">
      <section class="settings-group">
        ...
        <div class="settings-fields">
          ...
          <label>${t("transcribeEndpoint")}<input id="settingsTranscribeEndpointInput" /></label>
  ...
  $("settingsPresetBtn")?.addEventListener("click", () => {
    ...
    $("settingsTranscribeEndpointInput").value = $("transcribeEndpointInput").value;
...
async function saveSettings() {
  try {
    const transcribeEndpoint = $("transcribeEndpointInput").value.trim();
    const provider = await api("/api/settings/provider", {
      method: "PUT",
      body: JSON.stringify({
        chatEndpoint: $("chatEndpointInput").value.trim(),
        transcribeEndpoint: transcribeEndpoint === "浏览器实时识别，无需填写" ? "" : transcribeEndpoint,
        chatModel: $("chatModelInput").value.trim(),
        transcribeModel: $("transcribeModelInput").value.trim(),
...
function applyLanguageToStatic() {
  ...
  setText(".upload-button", t("upload"));
  const uploadInput = $("audioFile");
  if (uploadInput) document.querySelector(".upload-button")?.appendChild(uploadInput);
...
function syncSettingsPage(provider = state.provider || {}) {
  ...
  $("settingsTranscribeEndpointInput").value = provider.transcribeEndpoint || $("transcribeEndpointInput")?.value || "";
...
async function saveWorkspaceSettings(event) {
  ...
  $("transcribeEndpointInput").value = $("settingsTranscribeEndpointInput").value.trim();
...
function fillDoubaoPreset() {
  $("chatEndpointInput").value = "https://ark.cn-beijing.volces.com/api/v3/responses";
  $("chatModelInput").value = "doubao-seed-2-0-pro-260215";
  $("transcribeEndpointInput").value = "";
  $("transcribeModelInput").value = "Web Speech zh-CN";
...
function wireEvents() {
  ...
  on("workingPresetBtn", "click", fillDoubaoPreset);
  on("doubaoPresetBtn", "click", fillDoubaoPreset);
